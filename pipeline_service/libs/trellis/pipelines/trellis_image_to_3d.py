...
class TrellisImageTo3DPipeline(Pipeline):
    ...
    def run(
        ...
        *,
        ...
        # Adaptively adjust slat_steps based on voxel count
        default_steps = {**self.slat_sampler_params, **slat_sampler_params}.get('steps', 20)
        adaptive_steps = self.get_adaptive_slat_steps(coords, default_steps)
        slat_sampler_params = {**slat_sampler_params, 'steps': adaptive_steps}
    ...
    def get_adaptive_slat_steps(self, coords: torch.Tensor, default_steps: int) -> int:
        """
        Determine the number of slat steps based on voxel count.
        
        Args:
            coords (torch.Tensor): The coordinates of the sparse structure.
            default_steps (int): The default number of steps from parameters.
            
        Returns:
            int: The adjusted number of steps based on voxel count.
        """
        num_voxels = coords.shape[0]
        
        if num_voxels < 20000:
            steps = 30
            print(f"\033[92mVoxel count: {num_voxels} -> Using {steps} slat steps\033[0m")
        elif num_voxels < 35000:
            steps = 25
            print(f"\033[93mVoxel count: {num_voxels} -> Using {steps} slat steps\033[0m")
        else:
            steps = default_steps
            print(f"\033[94mVoxel count: {num_voxels} -> Using {steps} slat steps (default)\033[0m")
    ...
    def run_multi_image(
        ...
        # Adaptively adjust slat_steps based on voxel count
        default_slat_steps = {**self.slat_sampler_params, **slat_sampler_params}.get("steps", 20)
        adaptive_slat_steps = self.get_adaptive_slat_steps(coords, default_slat_steps)
        slat_sampler_params = {**slat_sampler_params, 'steps': adaptive_slat_steps}
        
        with self.inject_sampler_multi_image(
            "slat_sampler", len(images), adaptive_slat_steps, mode=mode
        ):
            slat = self.sample_slat(cond, coords, slat_sampler_params)
        return self.decode_slat(slat, formats)
