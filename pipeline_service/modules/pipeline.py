...
class GenerationPipeline:
    ...
    # --- HÀM CỐT LÕI 1A: CHUẨN BỊ ẢNH MULTI-VIEW (LEFT/RIGHT VIEWS) ---
    async def prepare_input_images(self, image_bytes: bytes, seed: int = 42) -> list[Image.Image]:
        """Chạy Qwen và RMBG để tạo 2 views (left/right). Dùng cho multidiffusion mode."""
        ...
        return [left_image_without_background, right_image_without_background]

    # --- HÀM CỐT LÕI 1B: CHUẨN BỊ ẢNH SINGLE (GOAT-10 STYLE) ---
    async def prepare_single_image(self, image_bytes: bytes, seed: int = 42) -> Image.Image:
        """Chạy Qwen và RMBG cho single image. Dùng cho single-image mode (goat-10 approach)."""
        image_base64 = base64.b64encode(image_bytes).decode("utf-8")
        image = decode_image(image_base64)
        if seed < 0:
            seed = secure_randint(0, 10000)
        ...
        # 1. Edit the image using Qwen Edit (default prompt from settings)
        image_edited = self.qwen_edit.edit_image(prompt_image=image, seed=seed)
    ...
    # --- HÀM CỐT LÕI 2A: CHẠY TRELLIS MULTI-IMAGE (MULTIDIFFUSION MODE) ---
    async def generate_trellis_only(self, processed_images: list[Image.Image], seed: int) -> bytes:
        """Chạy tạo 3D từ nhiều ảnh (multidiffusion mode)."""
    ...
    # --- HÀM CỐT LÕI 2B: CHẠY TRELLIS SINGLE-IMAGE (GOAT-10 STYLE) ---
    async def generate_trellis_single(self, processed_image: Image.Image, seed: int) -> bytes:
        """Chạy tạo 3D từ single image (goat-10 approach)."""
        trellis_params = TrellisParams.from_settings(self.settings)
        set_random_seed(seed)

        trellis_result = self.trellis.generate(
            TrellisRequest(
                images=[processed_image],  # Pass as single-item list
                seed=seed,
                params=trellis_params,
            )
        )
    ...
    async def generate_gs(self, request: GenerateRequest) -> GenerateResponse:
        ...
        return GenerateResponse(
            ...
            image_edited_file_base64=to_png_base64(processed_images[0]) if self.settings.send_generated_files else None,
            image_without_background_file_base64=to_png_base64(processed_images[0]) if self.settings.send_generated_files else None,
        )
